...
public class TemperatureExample {
    public static void main(String[] args) throws Exception {
        ...
        input
                .keyBy(1)
                .flatMap(new RichFlatMapFunction<Tuple3<Integer,Integer,Integer>, Tuple3<Integer, Integer, Integer>>() {
                    private final ValueStateDescriptor<Integer> count =
                            new ValueStateDescriptor<>("count", IntSerializer.INSTANCE, 0);


                    private final ReducingStateDescriptor<Integer> max =
                            new ReducingStateDescriptor<>("max",
                    ...
                    public void flatMap(Tuple3<Integer, Integer, Integer> value,
                        ...
                        if (value.f0 == 0) {
                            ValueState<Integer> countState =
                                    getRuntimeContext().getState(count);

                            countState.update(countState.value() + 1);

                            getRuntimeContext().getReducingState(max).add(value.f2);
                        } else if (value.f0 == 1) {
                            ReducingState<Integer> maxState =
                                    getRuntimeContext().getReducingState(max);
